...
public void cc_proc_RebuildString(const int mType, int iClient, int &pLevel, const char[] szBind, char[] szBuffer, int size)
{
	if(g_chIgnore[iClient][mType] && (!strcmp(szBinds[BIND_PROTOTYPE], szBind) || !strcmp(szBinds[BIND_MSG], szBind)) && pLevel < levels[szBind[1] == 'M'])
	{
		pLevel = levels[szBind[1] == 'M'];
		szBuffer[0] = 0;
	}
}
