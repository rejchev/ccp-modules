    ...
    switch(part) {
        ...
        case BIND_STATUS, BIND_STATUS_CO: {
            if(jConfig.HasKey(szBinds[part])) {
                jConfig.GetString(szBinds[part], value, size); // server is always alive :/

                if(part != BIND_STATUS) 
                    Format(value, size, "%T", value, props[2]);
                
                else Format(value, size, "%T", value, props[2], szBot);
            }
        }
        ...
        case BIND_PREFIX:{
            if(jConfig.HasKey(szBinds[part])) {
                jConfig.GetString(szBinds[part], value, size);
                Format(value, size, "%T", value, props[2], network);
            }
        }
        ...
        case BIND_MSG: {
            if(jConfig.HasKey(szBinds[part]) && (jValues = asJSONA(jConfig.Get(szBinds[part]))) && jValues.Length) {
                ...
                Format(value, size, "%c %T", 1, value, props[2], buffer);
            }
        }
