...
public void OnPlayerLoaded(int iClient, int iAccountID) {
    pckg_OnPackageAvailable(iClient, pckg_GetPackage(iClient));
}

public void pckg_OnPackageAvailable(int iClient, Handle hPkg) {
    if(!hPkg) {
        return;
    }
...
public void ccp_OnPackageUpdated(int iClient, Handle jsonObj, Handle hCaller) {
    ...
    pckg_OnPackageAvailable(iClient, jsonObj);
}
...
JsonObject ReadCookieValue(int iClient) {
    ...
    JsonArray objItems = asJSONA(asJSONO(pckg_GetPackage(0)).Get(pkgKey));
    JsonObject obj;
...
public void OnMapStart() {
    ...
    if(g_bLate) {
        g_bLate = false;
        pckg_OnPackageAvailable(0, pckg_GetPackage(0));
...
public int RankMenu_CallBack(Menu hMenu, MenuAction action, int iClient, int option) {
    switch(action) {
        ...
        case MenuAction_Select: {
            ...
            JsonObject  objModel    =   asJSONO(pckg_GetPackage(iClient));
...
public Processing cc_proc_OnNewMessage(const int[] props, int propsCount, ArrayList params) {
    ...
    senderModel = asJSONO(pckg_GetPackage(props[0]));
